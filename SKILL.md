...
**开工前核对四项：** ① 当前工具文档与已登录浏览器会话；② XHS 标题≤20字、抖音≤30字；
③ XHS 话题≤10个、抖音≤5个，并按页面实际限制复核；④ 两张独立封面保存后的真实构图与检测结果。
Codex 原生文件选择器已可自动上传视频及双封面；旧的“Codex 只能手动传封面”结论不再适用。
小红书裁剪默认值随编辑器版本变化，核对实际3:4构图，只有比例不符时才切换。

**用户规则: 视频时长 < 1 分钟不加章节** (两平台都不加)。抖音对短视频本来就不给章节入口。

## 当前工具能力与故障诊断

Codex 开始上传/交互前读取 [references/codex-publishing.md](references/codex-publishing.md)。
以本次工具返回的 API 为准，不照搬旧接口名。下文 `browser_batch`、页内 JS 修改/派发事件、
fetch、文件桥、window.name、IndexedDB 与系统剪贴板注入示例仅供 **Claude for Chrome 且工具允许时**
使用；Codex 按该参考用受支持的 UI/文件选择器完成相同业务步骤，`evaluate` 仅作只读 DOM 检查。
用户明确允许更合适工具时遵循其选择，但先验证能力，不因旧流程的失败推断新工具也做不到。

**无效操作及时诊断：** 同一动作两次无可见状态变化就停止原样重试，读当前状态和工具文档，检查
焦点、遮挡、加载、话题上限，再试一条受支持的替代路径。按钮转圈或处理中时先观察结果，不能连点提交。
`shadowRoot === null` 不能证明原生点击不可用；白屏或无效点击也不能证明掉登录。
已说“登录了”时先读现有标签，仅实际出现登录/验证界面才请用户处理。提交结果不明先查管理页。
...
3. 小红书传的同时回抖音填表(标题/正文/话题/声明)→ 抖音双封面按当前环境支持的文件上传路径处理
4. 抖音处理封面期间, 回小红书填表 + 封面注入 + 章节
...
## 发布前用户确认门 — 承接已有授权，只补缺失信息

先检查当前对话中的发布授权、指定 tags 和排期。以下两项尚未明确时再询问；已明确则沿用：
...
可合并复述最终清单作为执行说明，然后继续，**不要求用户再次批准同一内容**。只有缺少信息、指定 tag 超额需取舍，或要改变已授权内容/排期时才询问。

## 发布浏览器环境分流 — 硬规则

- **由 Codex 发布:** 小红书和抖音的视频上传、封面设置、表单填写、章节、定时和提交全部使用
  **Codex in-app Browser**（读取当前可用工具的入口文档） 中用户已经登录的标签页。不要改用
...
本人操作 — 白屏可能只是首屏还没挂完, 见 §2 慢启动); ③ 使用文件桥的旧路径才需要启动桥，原生文件选择器不需要。

**开工第一件事: 优先复用两平台现有标签确认登录态，无对应标签再新建**, 别等填到一半才发现。抖音掉登录很常见
...
**旧桥接路径的限制（仅对应旧工具版本）：** Codex 使用原生 filechooser，见4B；旧 `file_upload`
的10MB/目录限制不能套到当前文件选择器。Claude `file_upload` 的能力以当前文档和实际结果为准。
以下仅描述旧桥接方案，不代表当前平台上传能力：
...
2. **自主声明按实际制作方式选择**，AI画面/克隆配音使用对应AI声明，详见步骤5。
3. **视频≥1分钟时提供章节**: 用抖音智能章节自动生成, 或手动添加, 不能空着。
...
4. **封面 (两个槽) — 路径按环境分流, 先判断自己在哪个环境。**
   ...
   ### 4B. Codex in-app Browser → 原生文件选择器
   按 [Codex 上传流程](references/codex-publishing.md#视频与双封面上传)，点击当前槽位的真实上传按钮，
   通过 filechooser `setFiles` 传入项目绝对路径。竖、横两个槽分别上传对应文件；点“完成”保存后
   检查主表单缩略图，必要时重开确认实际保存内容。仅当前受支持上传路径确实失败时才交用户补该步。
...
6. **章节:** 智能章节和摘要先对照已批准口播核对事实、状态及时间，不能直接应用未审结果。章节名上限 **12 字**(超了"确定"不可点)。2026-08-02 实测**发布页的章节弹窗直接可用**
   ...
   - **⚠ 定时作品事后补章节大概率做不到 (2026-08-30 定论)**: 编辑页章节弹窗的预览播放器拉
     ...
     **别挂定时器反复试**。结论: 章节尽量在**首发的发布页**完成(发布页播放器吃客户端本地
     ...
     补(编辑作品→章节), 或接受该条无章节。
7. **定时发布:** 选择器用**浏览器本地时区** (默认=now+2h 本地)。使用 `Asia/Shanghai` 与浏览器实际 IANA 时区按目标日期换算，不能全年写死12小时差；
   保存用户北京时间、页面本地时间、时区和 UTC 时间点。
...
4. **话题 (⚠ 硬上限 10 个 — 2026-08-14 用户实测, 抖音是 5):** 文案里超出 10 个的只能自己取舍,
   ...
   会「加载中」>2s, 此时按 Enter = 白按, 残留纯文本)。验证口径: 只读检查 `a.tiptap-topic` 及其话题属性、逐个比对文本；纯文本 `#词` 不算成功。
   ...
   - **凑不齐时用编辑器下方那排「推荐话题」chips**: 随笔记预加载, 点一下立刻变真话题,
     完全不走搜索接口, 零等待。仅非用户指定项可用内容相关话题 (如 #网球) 替补；mandatory 项不能自行替换。
...
5. **⭐ 添加章节 (趁现在, 首发唯一窗口 —— 但 <1分钟的片子按用户规则直接跳过):**
   ...
   **「一键应用」**, 再点 **「保存章节」**。只看见建议列表或只点一键应用都不等于已落库, 回到主表单
   必须看到「已添加 N 个章节」。智能结果是未核查草稿：逐条对照已批准口播及真实时间，尤其检查“仍在进行/已经结束”等状态，
   防止把告别赛季写成已经退役。已有核验章节表时可直接手填，无需先等待智能生成。机械细节:
   ...
   - **最省事的加法(2026-08-02 定式): 在时间轴上随便点几个不同位置各加一行, 再逐行 triple_click
     ...
     相同时第二行加不上, 先改开再加。
   - **修改时间会即时重排行。** 改完分钟后先重读标题与 mm:ss，再定位同一行改秒，不能继续用旧 `nth`。
     首章00:00，其余满足页面最小间隔；全部保存前按标题→时间逐条读回，再核对主表单章节数量。
...
7. **封面:** 封面 b64 先经另一 XHS 标签存进 localStorage / IndexedDB → 表单页点第一块封面 tile →
   ...
   - 查看当前编辑器实际裁剪比例和最终预览。竖封面必须完整显示为3:4；若已正确无需重切，
     若默认4:3则在“裁剪/封面比例”改成3:4。不要仅凭输入图片尺寸或旧版默认值判断。
   - 保存出口可能叫“确定”或“完成”，按页面当前文字点击；保存后查看“封面效果评估通过”及真实缩略图。
...
9. **提交:** 点「定时发布」/「发布」→ 按钮转圈 **40–90s (常态, 签名超时所致)** → 成功 = 跳
   `/publish/success` 再回 `?published=true`。**URL 出现 `?published=true` 是提交回执**；还需管理页核对目标作品和准确排期，才能说定时已核验。

   **按钮无反应先诊断工具。** 按“当前工具能力与故障诊断”重读状态并使用当前支持的原生点击，
   检查权益弹窗是否遮挡；不从 closed shadow DOM 推断必须人工。仍无法触发时，完成其余字段后
   给用户具体剩余一步；不能重开/重传。任何提交结果不明先查精确作品记录，避免重复提交。
10. **验证 (笔记管理):** 打开笔记管理, 卡片上标「定时发布 YYYY-MM-DD HH:mm (GMT+8:00 北京时间)」
    ...
    - tab 归属**不稳定**: 2026-07-23 观察到定时笔记落在「审核中」; 2026-07-31 一批 7 篇实测
      ...
      **精确标题、时长和卡片定时标注**为准，计数仅作辅助。
    - 慢链路下**列表本身也可能加载不出来**(骨架屏/「加载中」不散), 而「全部 N」的计数会先出来 ——
      计数增量只能辅助判断；列表未读出时记录“已有提交回执，排期待核验”，不要重复提交或宣称排期已核验。
...
- 笔记管理「全部 N」计数可能显示 **0**(接口慢), 此时用 `?published=true` 记录提交回执，精确作品与排期仍待管理页核验。
...
| 笔记管理列表不出 | 记录已有回执，等待精确作品与排期核验；计数仅辅助 |
...
② 冷门新话题 (#伊埃拉夺冠 这种赛后几小时的新词) 搜索接口根本不回真结果, 退一格重触发
  也没用 — 非用户指定项可改用内容相关的推荐话题 chips；指定项不得替换，必要时让用户完成该话题选择, 别烧十几分钟等一个不会来的下拉。
...
- [ ] 已承接发布授权及用户明确的 tag 要求，缺失项已补齐；指定项完整保留，超额取舍已确认
- [ ] 立即/定时方式及精确北京时间已有明确指示，或缺失项已补齐；同一排期不重复请批
...
- [ ] XHS 章节(≥1 分钟): 若走智能生成, 已点「一键应用」+「保存章节」; 若手填, 也已保存;
      主表单最终显示「已添加 N 个章节」
- [ ] 封面: XHS 保存后实际3:4构图正确，标题及人物未被裁
- [ ] 声明: 按素材/配音实际制作方式设置两平台AI标识；原创声明按内容及用户授权填写
...
- [ ] XHS 最后一击: 无效点击已按当前工具文档诊断，提交不明先查管理页，不重开重填
- [ ] 工作标签全程没被关掉/导航 (点时间轴、弹窗等操作误关过 —— 关了按"关页恢复"流程查证再动)
- [ ] 提交后确认: XHS URL 变 `?published=true` + 笔记管理出现精确标题和北京时间定时标注;
      抖音内容管理出现精确标题、时长及排期，本地显示时间映射到同一北京时刻；审核状态按页面原样记录

## 4. 发布收尾与状态真源

逐平台保存结果，不能因为一个平台成功就把双平台一起标完成。`publish/plan.json` 作为发布状态
真源（已有别的真源则沿用并明确路径）：记录平台、精确标题、时长、提交回执、管理页URL、核验时间、
审核原文状态、用户北京时间、页面时间及时区、UTC时间点、实际话题清单、章节数和双封面检查结果。
状态区分“表单完成”“已提交待核验”“已核验定时待审核”“已发布”；“审核中”不等于审核已通过。
同时同步项目内已有 `production-status.json` 的发布字段及 BRIEF/README 中的状态摘要，或改为链接真源，
不保留相互矛盾的“未提交/已定时”。没有这些文件不必额外新建重复状态文件。

服务繁忙导致“作品检测失败”与内容审核拒绝分别记录，不能混为一谈。成功回执也不能代替精确排期
检查。收尾报告分别说清两平台实际状态、北京时间和待核验项；不自行增加定时监控或再次发布。
