...
(2026-07-30 定论, 当时环境)。
两条路都写在 §1 步骤4, 开工前先确认自己在哪个环境。

**开工前先读四条硬约束**: ① **抖音封面**: Claude for Chrome 走自动注入(双OS已验), Codex in-app 请用户手动传 (§1 步骤4);
② 小红书**标题上限 20 字**、抖音 30 字, 两边标题要分别裁 (§2);
③ **话题硬上限: 抖音 5 个 (§1 步骤3) / 小红书 10 个 (§2 步骤4)** — 文案里超出的按平台各自取舍,
   超额那个的表现是"下拉正常弹出但就是不变 chip", 别当成加载问题反复重试;
③b **小红书最后的「发布」按钮点不动是常态** (closed shadow DOM, §2 步骤9): 其余字段全部做完后
   请用户手点最后一下, 不要重开页面重填;
...
3. **话题 tags (抖音编辑器专属地雷):** 逐个 `type "#xx"` → 等 3s → `Enter`, **每个都要用
   ...
   - **最后一个话题后补一个空格**终结编辑态。
4. **封面 (两个槽) — 路径按环境分流, 先判断自己在哪个环境。**

   ### 4A. Claude for Chrome → **全自动, JS 注入可用** (Windows 2026-08-08 / macOS 2026-08-09 实测通过)
   两个槽都拿到「封面检测通过」且「横/竖双封面缺失」消失, 全程无人工。步骤:
   1. **封面 b64 先经 `window.name` 接力落到 `creator.douyin.com` 同源页 → `localStorage`**
      (抖音 CSP 拦页内 fetch 到 127.0.0.1, 必须接力; 用另开的空白标签, 别动工作标签)。
   2. 工作标签读出 → `new File([u8],name,{type:'image/jpeg'})` → 塞进 `input[accept*=image]`
      → 派发 `change`。
   3. **⚠ 一次只灌一个槽, 只对当前弹窗那一个 input 派发 change。**
      页面上有 5 个 `accept` 含 image 的 input; 图省事一次性全派发, **竖封面槽会变黑图**。
      2026-08-08 实测: 这就是"黑图"的成因 —— **是自伤, 不是平台的可信事件校验**。
      补灌一次竖版即恢复正常。
   ...
   5. **弹窗出口是右下角「完成」, 不是右上角 ×** (点 × 会弹「你有未保存的封面编辑效果, 是否关闭?」,
      点确定就丢了)。「完成」左边有「封面检测」, 点一下能拿到「封面检测通过 / 暂未发现封面低质问题」。
   6. 从表单里的封面 tile 点进去时是单槽模式(标题「设置竖封面」), 从头开时是双页签模式
      (顶部「设置竖封面 / 设置横封面」) —— 两种都按上面的规则走。
   - **`file_upload` 工具在 Claude for Chrome 里是坏的, 别浪费轮次**: `paths` 参数永远送不到,
     一律报 `expected array, received undefined`; 反斜杠/正斜杠/中文路径/纯 ASCII 短路径全试过。

   ### 4B. Codex in-app Browser → **必须用户手动传** (2026-07-30 定论)
   把两个槽的弹窗打开, 然后**停下来请用户上传**竖3:4 + 横4:3, 传完再继续。
   - **现象**: JS 把 File 塞进 input 并派发 change 后, 裁剪框里**图片正确渲染出来**(肉眼可见),
     但点「保存」后封面槽变成视频首帧的**纯黑图**。
   - **已逐个隔离排除的假设**: ① 拖裁剪框破坏裁剪区 → 完全不拖照样黑;
     ...
     (注: 4A 跑通后回看, ①②③ 的黑图很可能也有"一次性给全部 input 派发 change"的成分在里面。
     macOS 侧没有复测过 4A 的单槽注入手法 —— **下次在 Codex 环境不妨先按 4A 试一轮再退回手动**。)
   - 交接话术: 把弹窗开好, 给用户**绝对路径**, 说明传完告诉你; 其余字段(标题/正文/话题/声明/定时)
     你全自动做完, 只等这一步。

   - **小红书封面两个环境都不受影响**, JS 注入正常工作 (见 §2 步骤7)。
   - **成功标志(两条路共用)**: 两个 tile 都显示缩略图(竖槽不是黑的!) +
     `/封面检测通过/.test(body.innerText)` 且 `/双封面缺失/` 为 false。
5. **自主声明 (按内容性质选, 不是固定值):**
...
6. **章节:** 章节名上限 **12 字**(超了"确定"不可点)。2026-08-02 实测**发布页的章节弹窗直接可用**
   ...
   - **⚠ 定时作品事后补章节大概率做不到 (2026-08-30 定论)**: 编辑页章节弹窗的预览播放器拉
     ...
     blob 也不认), 「手动添加」被「请等待视频加载完成」永久门禁。跨 2 小时重试同样死,
     **别挂定时器反复试**。结论: 章节尽量在**首发的发布页**完成(发布页播放器吃客户端本地
     文件, 能加载; 大片要等转码, 见上一条), 实在赶不上就请用户在手机抖音 App 里 30 秒手动
     补(编辑作品→章节), 或接受该条无章节。
...
② 全新发布一条 (视频→标题→逐话题验 chip→声明→定时填原排期→双封面按 §1 步骤4 的环境分流做);
...
3. **正文:** tiptap/ProseMirror 很乖 — 一次 `ClipboardEvent('paste')` 贴整段, 换行保留:
   ...
   ```
   正文 = 文案正文, **不含章节块** (规范 1)。
4. **话题 (⚠ 硬上限 10 个 — 2026-08-14 用户实测, 抖音是 5):** 文案里超出 10 个的只能自己取舍,
   别硬塞。逐个 `type "#xx"` → **等下拉真正加载出条目** → 点击条目 (下拉是异步搜索, 冷门词
   会「加载中」>2s, 此时按 Enter = 白按, 残留纯文本)。验证口径: 数 innerText 里 `[话题]#` 个数。
   - **⚠ `computer type` 会静默吞掉 `#` (2026-08-14 实测)**: 打 `"#斯瓦泰克"` 落进编辑器的是
     `斯瓦泰克` — 无 `#` 就不触发下拉, 而且**看起来像"下拉没加载出来"**, 很容易误判成慢链路。
     正解: **用编辑器底部工具条的「# 话题」按钮插入 `#`**(JS 派发 5 连鼠标事件即可), 然后再
     `computer type` 打词。每加完一个话题, 顺手在同一个 JS 调用里再点一次「话题」按钮, 下一轮
     只剩"打字 + 选词"两步。判据: 点完按钮后 `ed.innerText` 尾部应出现 `#`。
...
9. **提交:** 点「定时发布」/「发布」→ 按钮转圈 **40–90s (常态, 签名超时所致)** → 成功 = 跳
   `/publish/success` 再回 `?published=true`。**URL 出现 `?published=true` 就是发布成功**,
   这是最可靠的判据。

   **⛔ 最后这一击点不动 —— 交给用户手点, 别空转 (2026-08-14 定论)。**
   底部按钮条是自定义元素 **`<xhs-publish-btn>`**(864×90, `position:fixed`), 内部是
   **closed shadow root**: `shadowRoot` 为 `null`、`children.length===0`、里面没有任何可被
   `querySelector('button')` 找到的节点。后果:
   - JS 派发(对 host 派发 pointerdown/mousedown/…/click, 带 `composed:true`)进不到内部按钮;
   - **`computer left_click` 打在按钮上也无效** —— 实测连点 4 次, 按钮**不转圈、不报错、URL 不变、
     表单原封不动**, 与"提交被弹窗吞掉"的表现完全一样, 极易误判成权益弹窗问题而反复重试。
   - 鉴别方法: `document.elementFromPoint(x,y).tagName === 'XHS-PUBLISH-BTN'` 且
     `host.shadowRoot === null` → 就是这条路, **立刻停手**。
   正确处置: 把**其余全部字段做完并核对**(标题/正文/话题/封面/章节/原创声明/定时), 然后把项目
   绝对路径和"只差点一下发布"讲清楚, 请用户手点; 用户点完按 `?published=true` + 笔记管理核验。
   **不要因为点不动就重开页面重填** —— 重填要再交 60–90s 首屏税且易丢字段。
   (注: 权益弹窗吞点击是**另一个**真实故障, 表现相同但可区分 —— 弹窗吞点击时原创声明 toggle 会
   弹回 off; 若 toggle 仍是红色 ON 而按钮无反应, 就是本条的 closed shadow DOM。)
...
- [ ] 话题: 数量对得上文案 (**XHS ≤10 个**, 数 `[话题]#`; 抖音末尾空格已补, **且 ≤5 个**)
...
- [ ] 封面: XHS 用竖版(自动注入), 评估通过; 抖音两个槽都有图(**竖槽不是黑的**)且「封面检测通过」
      —— Windows/Claude for Chrome 自动注入(一次一个槽), macOS/Codex 请用户手动传
...
- [ ] XHS 最后一击: 若「发布」按钮四次无反应且 toggle 仍 ON → closed shadow DOM, **交用户手点**,
      不重开页面重填
- [ ] 工作标签全程没被关掉/导航 (点时间轴、弹窗等操作误关过 —— 关了按"关页恢复"流程查证再动)
