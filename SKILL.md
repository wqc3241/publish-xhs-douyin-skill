...
**⚠ 环境决定封面能不能自动**: 抖音封面在 **Windows + Claude for Chrome 下可以 JS 注入自动完成**
(2026-08-08 实测), 在 **macOS/Codex in-app Browser 下必须用户手动传** (2026-07-30 定论)。
两条路都写在 §1 步骤4, 开工前先确认自己在哪个环境。

**开工前先读四条硬约束**: ① **抖音封面**: Windows/Claude for Chrome 走自动注入, macOS/Codex 请用户手动传 (§1 步骤4);
...
4. **封面 (两个槽) — 路径按环境分流, 先判断自己在哪个环境。**
   ...
   ### 4A. Windows + Claude for Chrome → **全自动, JS 注入可用** (2026-08-08 实测通过)
   ...
   ### 4B. macOS + Codex in-app Browser → **必须用户手动传** (2026-07-30 定论)
