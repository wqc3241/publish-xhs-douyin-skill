...
5. **⭐ 添加章节 (趁现在, 首发唯一窗口 —— 但 <1分钟的片子按用户规则直接跳过):**
   ...
   - **章节加在播放头位置**, 不是加一行空白让你填时间。所以顺序是
     **先在时间轴上点到目标秒 → 再点「+ 添加章节」**。播放头没动 = 同一秒已有章节 = 点了没反应。
   - 时间轴换算: 用刻度标签(00:05/00:25)反推 px/s, 再算目标 x。实测 26.6s 的片子约 9.75 px/s,
     点出来的秒数跟目标差 ≤1s, 可接受; 要精确可事后改行内的 mm/ss 输入框。
